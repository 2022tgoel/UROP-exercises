...
// g++ lesson_17*.cpp -g -I <path/to/Halide.h> -L <path/to/libHalide.so>
// -lHalide -lpthread -ldl -o lesson_17 -std=c++17
// LD_LIBRARY_PATH=<path/to/libHalide.so> ./lesson_17

// On os x:
// g++ lesson_17*.cpp -g -I <path/to/Halide.h> -L <path/to/libHalide.so>
// -lHalide -o lesson_17 -std=c++17 DYLD_LIBRARY_PATH=<path/to/libHalide.dylib>
// ./lesson_17
...
int main(int argc, char **argv) {
...

  {
    ...
    // Say we want an update that multiplies by two the values inside a circular
    // region centered at (3, 3) with radius of 3. To do this, we first define
    // the minimal bounding box over the circular region using an RDom.
    RDom r(0, 7, 0, 7);
      ...
      for (int x = 0; x < 7; x++) {
        if (halide_result(x, y) != c_result[y][x]) {
          printf("halide_result(%d, %d) = %d instead of %d\n", x, y,
                 halide_result(x, y), c_result[y][x]);
          return -1;
        }
      }
    }
      ...
      for (int x = 0; x < 5; x++) {
        if (halide_result_g(x, y) != c_result_g[y][x]) {
          printf("halide_result_g(%d, %d) = %d instead of %d\n", x, y,
                 halide_result_g(x, y), c_result_g[y][x]);
          return -1;
        }
      }
    }
